...
async def back_to_state_on_markup(
    message: types.Message, reply_text: str, markup, state_to_set
) -> None:
...
async def back_state_remove_roles_switcher(message: types.Message) -> None:
    await back_to_state_on_markup(
        message, "Возвращаемся назад!", kb_admin.admin_panel_menu, AdminState.settings
    )


async def back_state_roles(message: types.Message) -> None:
    await back_to_state_on_markup(
        message, "Возвращаемся назад!", kb_admin.roles_panel, AdminState.roles_switch
    )
